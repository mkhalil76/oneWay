  ...
  methods : {
      ...
      addNew() {
            ...
            vm.$validator.validate().then(valid => {
                vm.loading = true;
                if (valid) {
                    vm.$store.dispatch('resturants.newMenuItem', data).then((res) => {
                        vm.loading = false
                        if (this.$store.state.resturants.menu_item.errors) {
                            vm.postErrors = this.$store.state.resturants.menu_item.errors;

                            return;
                        } else {
                            Vue.$toast.open('New Menu Item added successfly');
                            this.$router.push({name: "menu items", params: { id: this.$route.params.id }})
                        }
