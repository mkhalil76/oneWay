...
<div class="animated fadeIn">
          <b-row>
            <b-col sm="12">
                ...
                <b-form autocomplete="off" @submit.prevent="update" method="post">
                ...
                <b-form-group>
                    ...
                    <b-form-select
                        ...
                        :plain="true">
                        <option v-for="category in categories" v-bind:key="category.id" :value="category.id" :selected="resturant.category_id">
                            {{ category.name }}
                        </option>
                    </b-form-select>
                ...
                <b-form-group>
                    <label for="country">City  <span>*</span></label>
                        <b-form-select
                            ...
                            :plain="true">
                              <option  v-for="city in citieslist" v-bind:key="city.id" :value="city.id">
                                  {{ city.name }}
                              </option>
                        </b-form-select>
                ...
                <b-form-group>
                    <label for="country">Location  <span>*</span></label>
                        <b-form-select
                            ...
                            :plain="true">
                              <option  v-for="location in locationList" v-bind:key="location.id" :value="location.id">
                                  {{ location.name }}
                              </option>
                        </b-form-select>
  ...
  methods : {
      ...
      update() {
            ...
            vm.$validator.validate().then(valid => {
                vm.loading = true;
                if (valid) {
                    vm.$store.dispatch('resturants.update', data).then((res) => {
                        vm.loading = false
                        if (this.$store.state.resturants.active.errors) {
                            ...
                            Vue.$toast.open('Resturant updated successfly');
